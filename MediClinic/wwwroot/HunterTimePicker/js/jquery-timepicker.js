    ...
    $.hunterTimePicker = function(box, options) {
        ...
        function hourEvent() {
            time_wrap.on('click', '.Hunter-hour', function (event) {
                ...
                var inputTargetId = box.attr('id');
                if (inputTargetId == "DriverDto_WorkingStartTime") {
                    $("#workStartTimeValidation").html("");
                }
                else if (inputTargetId == "DriverDto_WorkingEndTime") {
                    $("#workEndTimeValidation").html("");
                }

                if(temp.length > 1) {
                    box.val(hourValue + ":" + temp[1]);
                } else {
                    var newvalue = hourValue + ":00";
                    if (newvalue == "00:00") {
                        newvalue = "";
                    }
                    box.val(newvalue);
